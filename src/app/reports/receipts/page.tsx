...
export default function ReceiptRegisterPage() {
  ...
  // Export CSV
  function exportCSV() {
    const headers = ['Date', 'Voucher No', 'Deposited To (Bank/Cash)', 'Received From (Customer)', 'Narration', 'Amount (OMR)']
    const dataRows = vouchers.map(v => {
      // Find the debit bank/cash account
      const drLine = v.journal_lines?.find((line: any) => line.type === 'Dr')
      const ledgerName = drLine?.ledger?.name || 'Bank/Cash'
      return [
        v.date,
        v.voucher_number || '—',
        ledgerName,
    ...
    link.setAttribute('download', `Receipt_Register_${fromDate || 'Start'}_to_${toDate}.csv`)
  ...
  return (
    <div>
      ...
      <div className="page-header">
        <div>
          <h1 className="page-title">Receipt Register</h1>
          <p className="page-subtitle">Historical registry of all corporate receipts and bank deposits</p>
        </div>
        ...
        <div style={{ padding: '0 1.75rem 1.25rem', borderBottom: '1px solid var(--color-border-light)', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '1rem', marginBottom: '1rem' }}>
          <div>
            <h2 style={{ fontSize: '1.1rem', fontWeight: 800, color: 'var(--color-teal)' }}>
              Corporate Inflow Registry (Receipts)
            </h2>
          ...
          <table>
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={6} className="table-empty">No receipts found.</td></tr>
              ) : (
                <>
                  {vouchers.map(v => {
                    const drLine = v.journal_lines?.find((line: any) => line.type === 'Dr')
